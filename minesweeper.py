        ...
        for i in range(cell[0], -1, cell[0] + 2 ):
            for j in range(cell[1], -1, cell[1] + 2 ):
                if (i,j) in self.mines:
                    countMines += 1
                if self.height > i >= 0 and self.width > j >= 0 and (i,j) not in self.safes and (i,j) not in self.mines:
                    unditerminedCells.append((i,j))

        newSentence = Sentence(unditerminedCells, count - countMines)
